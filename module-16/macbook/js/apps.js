...
function promocode() {
  ...
  const totalPrice = document.getElementById("total-price");
  const totalCost = parseInt(totalPrice.textContent);

  if (promoInput === validPromo) {
    const discountedPrice = totalCost - (totalCost * discountPercent) / 100;
    totalPrice.textContent = discountedPrice > 0 ? discountedPrice : 0; 
